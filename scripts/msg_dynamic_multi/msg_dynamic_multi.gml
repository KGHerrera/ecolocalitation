...
function msg_dynamic_multi() {
	...
	// Si no existe, inicialízala con el valor proporcionado
	var array = argument[0];
	
	var next = keyboard_check_pressed(ord("X")) or gamepad_button_check_pressed(0, gp_face3) or gamepad_button_check_pressed(0, gp_shoulderl)
	...
	var msg_total = array_length(argument[0]);
	if (!variable_instance_exists(id, "line")) line = 0;
	//This is the speed at which the text is drawn. You may change it:
	var text_speed = 0.25;
	
	if (keyboard_check(ord("X")) or gamepad_button_check(0, gp_face3) or gamepad_button_check(0, gp_shoulderl)){
	...
	//message complete
	return (char >= string_length(msg_org) and array[line] == array[array_length(array)-1] and (keyboard_check_pressed(ord("X")) or gamepad_button_check_pressed(0, gp_face3) or gamepad_button_check_pressed(0, gp_shoulderl)))
